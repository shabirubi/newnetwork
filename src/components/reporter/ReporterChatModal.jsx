...
export default function ReporterChatModal({ reporter, onClose, isOpen = true }) {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const response = await base44.integrations.Core.InvokeLLM({
        prompt: `אתה ${reporter.name}, ${reporter.role} מומחה ב${reporter.specialty}.
        
המשתמש שואל: "${userMessage}"

תן תשובה מקצועית, ידידותית וממוקדת (2-4 משפטים). ענה כאילו אתה באמת הכתב ${reporter.name}. השב בעברית בלבד.`,
            ...
            <div className="flex justify-start gap-3">
              ...
              <div className="bg-white dark:bg-gray-800 rounded-2xl p-3 shadow-md">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-gray-500">{reporter.name} כותב...</span>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <p className="text-xs text-gray-600 dark:text-gray-400">
              צ'אט AI עם {reporter.name} - מומחה ב{reporter.specialty}
            </p>
          </div>
