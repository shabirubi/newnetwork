      ...
      <div className="max-w-6xl mx-auto px-3 sm:px-4">
        ...
        <div className="flex items-center justify-between mb-3 sm:mb-4">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-black/80 flex items-center justify-center border border-white/20">
              <Radio className="w-4 h-4 sm:w-5 sm:h-5 text-white" />
            </div>
            <h2 className="text-base sm:text-2xl font-bold text-white">דיווחים בזמן אמת</h2>
          </div>
          <button
            onClick={() => refetch()}
            className="p-2 rounded-lg bg-black/60 hover:bg-black/80 text-white transition-colors border border-white/10"
            title="רענן"
          >
            ...
            <AnimatePresence>
              {alerts.map((alert, index) => (
                <motion.div
                  ...
                  className="bg-black/90 border border-white/10 rounded-xl p-3 sm:p-5 hover:border-white/20 transition-all"
                >
                  {/* Badge */}
                  {alert.is_breaking && (
                    <div className="absolute top-2 right-2">
                      <motion.span
                        ...
                        className="inline-block px-2 py-0.5 bg-red-600 text-white text-[10px] font-bold rounded"
                      >
                        🔴 חם
                      </motion.span>
                    </div>
                  ...
                  <div className="space-y-2">
                    <h3 className="font-bold text-white leading-tight line-clamp-2 text-sm sm:text-base pr-12">
                      {alert.title}
                    </h3>
                    ...
                    <div className="flex items-center gap-1 text-[10px] sm:text-xs text-gray-500">
                      <span className="truncate max-w-[100px]">{alert.source || 'הרשת החדשה'}</span>
                      <span>•</span>
                    ...
                    <div className="flex items-center gap-2 pt-2 border-t border-white/10">
                      <button
                        ...
                        className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 bg-black/60 hover:bg-black/80 disabled:bg-gray-800 text-white rounded-lg font-medium transition-colors text-xs sm:text-sm border border-white/10"
                      >
                        {loadingId === alert.id ? (
