...
export default function WeatherAlertsContainer() {
  ...
  return (
    <motion.div
      ...
      className="bg-black/90 rounded-2xl p-3 sm:p-4 border border-gray-800 shadow-lg overflow-hidden relative w-full max-w-full"
    >
      {/* Content */}
      <div className="relative z-10 space-y-3 sm:space-y-4">
        ...
        <div
          className="grid grid-cols-3 gap-2 lg:grid-cols-6 lg:gap-3 w-full"
        >
          {mockWeatherMetrics.map((metric, idx) => {
            ...
            return (
              <div
                key={idx}
                className="bg-gray-900 rounded-lg p-2 lg:p-3 border border-gray-800 hover:border-gray-700 transition-all cursor-pointer group w-full min-w-0 overflow-hidden"
              >
                <div className={`${metric.color} mb-1 lg:mb-1.5 group-hover:scale-110 transition-transform flex justify-center`}>
                  <Icon className="w-4 h-4 lg:w-5 lg:h-5" />
                </div>
                <p className="text-gray-400 text-[9px] lg:text-[10px] font-semibold mb-0.5 text-center truncate">{metric.label}</p>
                <p className="text-white font-bold text-[11px] lg:text-sm text-center truncate">{metric.value}</p>
              </div>
            );
          })}
        </div>
