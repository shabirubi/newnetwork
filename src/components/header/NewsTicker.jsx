  ...
  return (
    <div className="bg-black dark:bg-gray-950 text-white overflow-hidden relative z-[35] w-full">
      ...
      <div className="bg-gradient-to-r from-gray-900 via-black to-gray-900 overflow-hidden border-t border-red-900/20 h-8 w-full">
        <motion.div
          className="flex whitespace-nowrap h-full items-center text-lg"
          animate={{ x: [0, -2000] }}
        ...
        >
          {Array(4).fill([
            ...
            <span 
              key={`broadcast-${idx}`} 
              className="mx-8 flex-shrink-0 font-bold text-base" 
              style={{
...
function UpdatesStrip() {
  ...
  return (
    <div className="bg-gradient-to-r from-black via-gray-900 to-black overflow-hidden border-t border-red-900/20 h-9 w-full">
      <motion.div
        className="flex whitespace-nowrap h-full items-center text-sm gap-12"
        animate={{ x: [`0vw`, `-${updates.length * 300}px`] }}
        transition={{ duration: updates.length * 8, repeat: Infinity, ease: "linear" }}
      >
        {[...updates, ...updates].map((update, idx) => (
          <Link 
            key={`update-${idx}`}
            to={createPageUrl(`Article?id=${update.id}`)}
            className="flex items-center gap-3 hover:text-[#E31E24] transition-colors flex-shrink-0"
          >
