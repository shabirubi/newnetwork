...
export default function ReporterAudioPlayer({ reporter, article, onClose }) {
  ...
  const generateAndPlayAudio = async () => {
    ...
    try {
      // Create narration text with STRICT gender-specific language
      const genderText = reporter.gender === 'female' ? 'כתבת' : 'כתב';
      
      const narrationPrompt = reporter.gender === 'female' ? `
אתה חייב לכתוב בלשון נקבה בלבד!

צור קריינות חדשותית מקצועית עבור כתבת (אישה).

מבנה הקריינות:
1. פתיחה: "שלום, אני ${reporter.name}, ${reporter.role} מחדשות הרשת החדשה"
2. תוכן: סיפור דרמטי על הכתבה (15-25 שניות)
3. סיום: "נתראה בהמשך"
...
חוקים קריטיים:
- כל הפעלים בלשון נקבה: "עמדתי", "דיברתי", "ראיתי", "שאלתי"
- תארים בלשון נקבה: "מוכנה", "עומדת", "נמצאת"
- אסור להשתמש בפעלים של זכר!
- דוגמאות נכונות: "אני כאן בשטח", "דיברתי עם", "ראיתי את"
- דוגמאות שגויות: "אני עומד", "דיברתי עם" (זכר)

החזר רק טקסט קריינות בלשון נקבה בלבד.
` : `
אתה חייב לכתוב בלשון זכר בלבד!

צור קריינות חדשותית מקצועית עבור כתב (גבר).
...
חוקים קריטיים:
- כל הפעלים בלשון זכר: "עמדתי", "דיברתי", "ראיתי", "שאלתי"
- תארים בלשון זכר: "מוכן", "עומד", "נמצא"
- אסור להשתמש בפעלים של נקבה!
- דוגמאות נכונות: "אני כאן בשטח", "דיברתי עם", "ראיתי את"

החזר רק טקסט קריינות בלשון זכר בלבד.
`;
