...
Deno.serve(async (req) => {
  try {
    ...
    const { imageUrl, text, title, voice } = await req.json();

    if (!imageUrl || !text || !voice) {
    ...
    // Limit text to reasonable length (prevent API issues)
    ...
    // 2. Create talking avatar video with D-ID
